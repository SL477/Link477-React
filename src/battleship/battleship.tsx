  ...
  checkValid() {
    ...
    for (let i = 0; i < this.positions.length; i++) {
      if (
        ...
        this.positions[i].x > MAX ||
        this.positions[i].y > MAX
      ) {
        return false;
      }
    }
...
class Gameboard {
    ...
    for (let i = 0; i < this.ships.length; i++) {
      ...
      for (let j = 0; j < this.ships[i].positions.length; j++) {
        ...
        for (let s = 0; s < newShip.positions.length; s++) {
          ...
          if (
            newShip.positions[s].x == this.ships[i].positions[j].x &&
            newShip.positions[s].y == this.ships[i].positions[j].y
          ) {
  ...
  receiveAttack(x: number, y: number) {
    for (let i = 0; i < this.ships.length; i++) {
      if (this.ships[i].hit(x, y)) {
        ...
        return {
          result: 'HIT',
          msg: didsink ? 'Sunk ' + this.ships[i].type : 'Hit',
        };
      }
    }
  ...
  getStats() {
    ...
    return {
      ...
      numSunkShips: this.ships.filter((ship) => {
        return ship.sunk;
      }).length,
        ...
        <>
          {changeOver ? (
            ...
            <>
              ...
              <div className="container">
                <div className={classes.board}>
                  <table className={classes.grid}>
                    <thead>
                      <tr>
                        <th colSpan={10} className={classes.center}>
                          Your Board
                        </th>
                      </tr>
                    </thead>
                    <tbody>
                      {(currentPlayer === 1 ? player1 : player2).gameBoard
                        .getCurrentGrid()
                        .map((arr, rowIndex) => {
                          return (
                            <tr key={rowIndex}>
                              {arr.map((a, colIndex) => {
                                return (
                                  <td
                                    key={colIndex}
                                    className={`${a.ship ? classes.ship : ''} ${a.hit ? classes.hit : ''} ${classes.td}`}
                                  >
                                    {a.hit
                                      ? 'X'
                                      : a.shiptype
                ...
                <div className={classes.board}>
                  <table className={classes.grid}>
                    ...
                    <tbody>
                      {(currentPlayer === 1 ? player2 : player1).gameBoard
                        .getCurrentGrid()
                        .map((arr, rowIndex) => {
                          return (
                            <tr key={rowIndex}>
                              {arr.map((a, colIndex) => {
                                return (
                                  <td
                                    ...
                                    className={
                                      a.hit
                                        ? `${classes.hit} ${classes.td}`
                                        : classes.td /* + (a.ship? ' ship' : '')*/
                                    }
                                    onClick={() => {
                                      if (!a.hit) {
                                        sendAttack(colIndex, rowIndex);
                                      }
                                    }}
                                  ></td>
                                );
              ...
              <p>
                <b>Number of own ships:</b>{' '}
                {(currentPlayer === 1 ? player1 : player2).gameBoard.getStats()
                  .numShips -
                  (currentPlayer === 1 ? player1 : player2).gameBoard.getStats()
                    .numSunkShips}
              </p>
              <p>
                <b>Number of opponent ships:</b>{' '}
                {(currentPlayer === 2 ? player1 : player2).gameBoard.getStats()
                  .numShips -
                  (currentPlayer === 2 ? player1 : player2).gameBoard.getStats()
                    .numSunkShips}
              </p>
